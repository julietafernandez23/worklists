...
definePage({
  meta: {
    title: 'Collections view modes',
    description:
      'Saved collections with switchable grid, list, and contributor views for browsing and editing.',
    group: 'other',
  },
})
...
const savedCollectionLink = computed(() => ({
  path: '/collections-view-modes/saved',
    ...
    <main>
      ...
      >
        <template #bookmark-menu="{ articleTitle, close }">
          <div class="cvm-bookmark-menu">
            <div class="cvm-bookmark-menu__header">
              <div class="cvm-bookmark-menu__header-top">
                <div class="cvm-bookmark-menu__status">
                  <span class="cvm-bookmark-menu__check" aria-hidden="true">
                    <CdxIcon :icon="cdxIconCheck" />
                  </span>
                  <p class="cvm-bookmark-menu__message">
                    {{ articleTitle }} added to
                    <a href="#" class="cvm-bookmark-menu__saved-link" @click.prevent>saved items</a>
                  </p>
                </div>
                <CdxButton
                  class="cvm-bookmark-menu__close"
                  weight="quiet"
                  :icon-only="true"
                  aria-label="Close"
                  @click="close"
              ...
              <button
                type="button"
                class="cvm-bookmark-menu__create"
            ...
            <ul class="cvm-bookmark-menu__list" role="list">
              <li
                v-for="collection in COLLECTIONS"
                :key="collection"
                class="cvm-bookmark-menu__item"
              >
                <span class="cvm-bookmark-menu__item-label">{{ collection }}</span>
                <CdxButton
                  class="cvm-bookmark-menu__item-add"
                  weight="quiet"
                  :icon-only="true"
      ...
      <div
        ...
        class="cvm-added-toast"
        role="status"
        aria-live="polite"
      >
        <span class="cvm-added-toast__check" aria-hidden="true">
          <CdxIcon :icon="cdxIconCheck" />
        </span>
        <p class="cvm-added-toast__message">
          {{ ARTICLE_TITLE }} has been added to
          <RouterLink :to="savedCollectionLink" class="cvm-added-toast__collection">
            {{ addedToCollection }}
          </RouterLink>
        </p>
      </div>
...
.cvm-bookmark-menu__check :deep(.cdx-icon) {
  width: 0.6875rem;
  height: 0.6875rem;
  color: var(--color-inverted-fixed, #fff);
}

.cvm-bookmark-menu__message {
  margin: 0;
...
.cvm-bookmark-menu__saved-link {
  color: var(--color-progressive);
  font-weight: var(--font-weight-bold);
  text-decoration: none;
}

.cvm-bookmark-menu__saved-link:hover {
  text-decoration: underline;
}

.cvm-bookmark-menu__close {
  flex-shrink: 0;
  margin: calc(-1 * var(--spacing-25)) calc(-1 * var(--spacing-25)) 0 0;
}

.cvm-bookmark-menu__create {
  display: inline-flex;
  align-items: center;
  gap: var(--spacing-35);
  width: calc(100% + 2 * var(--spacing-150));
...
.cvm-bookmark-menu__create:hover {
  text-decoration: underline;
}

.cvm-bookmark-menu__create :deep(.cdx-icon) {
  color: var(--color-progressive);
}

.cvm-bookmark-menu__list {
  list-style: none;
  margin: 0;
  padding: 0;
}

.cvm-bookmark-menu__item {
  display: flex;
  align-items: center;
  gap: var(--spacing-50);
...
.cvm-bookmark-menu__item:last-child {
  border-bottom: none;
  padding-bottom: var(--spacing-75);
}

.cvm-bookmark-menu__item-label {
  flex: 1 1 auto;
  min-width: 0;
  font-family: var(--font-family-system-sans);
  font-size: var(--font-size-medium);
...
.cvm-bookmark-menu__item-add :deep(.cdx-icon) {
...
.cvm-added-toast__check :deep(.cdx-icon) {
  width: 0.6875rem;
  height: 0.6875rem;
  color: var(--color-inverted-fixed, #fff);
}

.cvm-added-toast__message {
  margin: 0;
  flex: 1 1 auto;
  min-width: 0;
  font-family: var(--font-family-system-sans);
...
.cvm-added-toast__collection {
  color: var(--color-progressive);
  font-weight: var(--font-weight-bold);
  text-decoration: none;
}

.cvm-added-toast__collection:hover {
  text-decoration: underline;
}
</style>
