...
definePage({
  meta: {
    title: 'Worklist (compact list)',
    description:
      'View the worklist as a compact list with dividers, with an MVP of article context: views per month, edit suggestions, and article quality.',
  },
})
...
<template>
  <ChromeWrapper :last-edited-notice="false" skin="mobile">
    <SpecialPageWrapper :title="null" class="wc3__special-page" skin="mobile">
      <template #header>
        <div class="wc3__page-title">
          <h1 class="wc3__page-title-text">
          ...
          <CdxMenuButton
            ...
            class="wc3__page-title-menu"
          >
            <CdxIcon :icon="cdxIconEllipsis" />
          </CdxMenuButton>
        </div>
      </template>
      <CdxTabs v-model:active="activeTab" class="wc3__tabs">
        ...
        <CdxTab name="worklist" label="Worklist">
          <div class="wc3__page">
            <div v-if="loading" class="wc3__loading">Loading articles…</div>

            <ul v-else class="wc3__list" role="list">
              <li v-for="item in items" :key="item.title" class="wc3__item">
                <a
                  class="wc3__item-title"
                  :href="item.url"
                  target="_blank"
                  rel="noopener noreferrer"
                >{{ item.title }}</a>
                <p class="wc3__item-description">{{ item.description }}</p>

                <div class="wc3__meta-row">
                  <span class="wc3__meta-item">
                    <CdxIcon :icon="cdxIconChartLine" size="small" class="wc3__meta-icon wc3__meta-icon--views" />
                    <span>{{ item.viewsPerMonth }}</span>
                  </span>
                  <span class="wc3__meta-item">
                    <CdxIcon :icon="cdxIconLightbulb" size="small" class="wc3__meta-icon wc3__meta-icon--suggestions" />
                    <span>{{ suggestionLabel(item.suggestionCount) }}</span>
                  </span>
                  <span
                    class="wc3__meta-item wc3__meta-item--quality"
                    :class="`wc3__meta-item--${item.quality}`"
                  >
                    <CdxIcon
                      v-if="item.quality === 'high'"
                      :icon="cdxIconArrowUp"
                      size="small"
                      class="wc3__meta-icon"
                    />
                    <CdxIcon
                      ...
                      class="wc3__meta-icon"
                    />
                    <span v-else class="wc3__meta-icon wc3__meta-icon--medium" aria-hidden="true">—</span>
...
<style scoped>
.wc3__special-page:deep(.special-page-wrapper__header) {
  width: 100%;
}

.wc3__special-page:deep(.special-page-wrapper__title-cluster) {
  flex: 1 1 0;
  min-width: 0;
  width: 100%;
}
...
.wc3__item:first-child {
  padding-top: 0;
}

.wc3__item:last-child {
  border-bottom: none;
  padding-bottom: 0;
}
...
.wc3__item-title:hover {
  text-decoration: underline;
}

.wc3__item-description {
...
.wc3__meta-icon--views {
  color: var(--color-subtle);
}

.wc3__meta-icon--suggestions {
  color: var(--color-progressive);
}

.wc3__meta-icon--medium {
  display: inline-flex;
  align-items: center;
...
.wc3__meta-item--quality.wc3__meta-item--high,
.wc3__meta-item--quality.wc3__meta-item--high .wc3__meta-icon {
  color: var(--color-success);
}

.wc3__meta-item--quality.wc3__meta-item--medium {
  color: var(--color-warning);
}

.wc3__meta-item--quality.wc3__meta-item--low,
.wc3__meta-item--quality.wc3__meta-item--low .wc3__meta-icon {
